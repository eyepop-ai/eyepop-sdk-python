...
- `EyePopPointCloudPlot` scatters point clouds into a 3D axes, one colour and legend entry per object, in metres. The point budget is shared across clouds so adding objects thins the scatter rather than multiplying it, and the box takes the data's own proportions, since an unequal box would misrepresent the geometry the coordinates exist to measure. The axes are labelled but not reoriented: with extrinsics the metres are world coordinates with Z up, without them they are camera coordinates with Y down, and which one a prediction holds is not recoverable from it. `pop_demo.py --visualize-point-cloud` renders every cloud in the results this way.
