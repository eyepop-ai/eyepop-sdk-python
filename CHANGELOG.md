...
- Streaming starts immediately rather than waiting for the camera's first capture time, so the leading frames are relayed without one and their predictions carry no `captured_at`. This is what a directly connected camera does too, which is the point: matching it keeps the two paths indistinguishable through startup rather than only once they settle. A camera that supplies no usable clock at all is reported once, naming the camera's missing sender reports as the cause, and only after long enough that a healthy stream's startup window cannot trigger it - a warning that fires on every stream would hide the one case it exists to report.
