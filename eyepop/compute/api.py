    ...
    if need_new_session:
        try:
            ...
            async with client_session.post(sessions_url, headers=headers) as post_response:
                ...
                log.debug(f"POST /v1/sessions response: {res.status}")
        except aiohttp.ClientResponseError as e:
            raise ComputeSessionException(
...
async def refresh_compute_token(
    ...
    if not compute_config.session_uuid:
        raise ComputeTokenException("Cannot refresh token: no session_uuid in compute_config")
    ...
    headers = {"Authorization": f"Bearer {compute_config.api_key}", "Accept": "application/json"}
    ...
    try:
        async with client_session.post(refresh_url, headers=headers) as response:
            ...
            log.debug(
                f"Token refreshed successfully, expires in: {compute_config.access_token_expires_in}s"
            )
            return compute_config
        ...
        log.debug(str(e))
        raise ComputeTokenException(
            f"Token refresh failed: {str(e)}", session_uuid=compute_config.session_uuid
        ) from e
