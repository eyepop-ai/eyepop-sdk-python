...
def fetch_new_compute_session(compute_config: ComputeContext) -> ComputeContext:
    ...
    log.info(f"Fetching sessions from: {compute_config.compute_url}/v1/sessions")
    
    # Initialize res variable for later use
    res = None
    ...
    try:
        ...
        log.info(f"GET /v1/sessions response: {res}")
        
        # Check if we need to create a new session
        if not res:
            need_new_session = True
            log.info("Response is empty/None, need to create new session")
        elif isinstance(res, list) and len(res) == 0:
            need_new_session = True
            log.info("Response is empty list, need to create new session")
        elif isinstance(res, dict) and not res.get('session_uuid'):
            need_new_session = True
            log.info("Response is dict without session_uuid, need to create new session")
            
    except requests.HTTPError as e:
        if e.response.status_code == 404:
            need_new_session = True
            log.info("GET /v1/sessions returned 404, will create new session")
        else:
    ...
    if need_new_session:
        try:
            log.info(f"Creating new session via POST to: {compute_config.compute_url}/v1/sessions")
    ...
    log.info(f"Session endpoint: {session_response.session_endpoint}")
    log.info(f"Session UUID: {session_response.session_uuid}")
    log.info(f"Access token present: {bool(session_response.access_token)}")
    log.info(f"Pipelines: {session_response.pipelines}")
